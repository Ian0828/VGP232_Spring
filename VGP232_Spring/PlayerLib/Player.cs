...
namespace PlayerLib
{
    ...
    public class Player
    {
        ...
        public static bool TryParse(string rawData, out Player player)
        {
            ...
            if (values.Length == 10)
            {
                try
                {
                    ...
                    player.Position = Enum.Parse<PlayerPosition>(values[2]);
        ...
        {
            return right.Shooting.CompareTo(left.Shooting);
        }
        ...
        {
            return string.Format("{0}, Overall: {1}, {2}, Shooting: {3}, Passing: {4}, Speed: {5}, Vertical: {6}, Dribble: {7}, Height: {8}cm, Weight: {9}kg", Name, Overall, Position, Shooting, Passing, Speed, Vertical, Dribble, Height, Weight);
        }
